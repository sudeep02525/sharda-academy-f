...
import { 
  ...
  ContactInfoEditor, 
  OfficeHoursEditor 
} from "@/cms/components/cms/contactpage";

const DEFAULT_DATA = {
  hero: { badge: "", title: "", description: "" },
  contactInfo: { phone: "", emergency: "", email: "", address: "", mapQuery: "" },
  officeHours: []
};
  ...
  return (
    <PageContainer>
      <div className="flex flex-col lg:flex-row gap-6 mb-8 items-start lg:items-center justify-between animate-fade-in-up">
        <div>
          <h1 className="text-2xl font-black text-navy uppercase tracking-tight">Manage Contact Us</h1>
          <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">
            Edit contact information, address, and office hours
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            {activeTab === "info" && (
              <ContactInfoEditor 
                ...
                onChange={(newData) => handleDataChange("contactInfo", newData)} 
              />
            )}
            
            {activeTab === "hours" && (
              <OfficeHoursEditor 
                data={data.officeHours} 
                onChange={(newData) => handleDataChange("officeHours", newData)} 
              />
            )}
