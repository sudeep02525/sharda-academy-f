...
import { 
  HeroEditor, WhyShardaEditor, PopularCoursesEditor, 
  LearningProcessEditor, FacultyEditor, AchieversEditor, 
  TestimonialsEditor, AdmissionProcessEditor, AnnouncementsEditor, 
  FAQEditor, ContactCTAEditor 
} from "@/cms/components/cms/homepage";

const TABS = [
  { id: "hero", label: "Hero Section" },
  { id: "why", label: "Why Sharda" },
  { id: "courses", label: "Popular Courses" },
  { id: "learning", label: "Learning Process" },
  { id: "faculty", label: "Faculty Preview" },
...
export default function HomepageCMS() {
  ...
  const handleGlobalSave = () => {
    setIsSaving(true);
    // Dispatch custom event to trigger save in active child tab
  ...
  return (
    <PageContainer>
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-6 items-start lg:items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-navy uppercase tracking-tight">Manage Homepage</h1>
          <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">Control the landing page content & layout</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-64 shrink-0">
          <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm sticky top-24">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-3 mb-3">Homepage Sections</h3>
        ...
        <div className="flex-1 min-w-0">
          <div className="animate-fade-in-up">
            ...
            {activeTab === "why" && <WhyShardaEditor />}
            {activeTab === "courses" && <PopularCoursesEditor />}
            {activeTab === "learning" && <LearningProcessEditor />}
            {activeTab === "faculty" && <FacultyEditor />}
            {activeTab === "achievers" && <AchieversEditor />}
            {activeTab === "testimonials" && <TestimonialsEditor />}
            {activeTab === "admission" && <AdmissionProcessEditor />}
            {activeTab === "announcements" && <AnnouncementsEditor />}
            {activeTab === "faq" && <FAQEditor />}
            {activeTab === "contact" && <ContactCTAEditor />}
          </div>
        </div>
