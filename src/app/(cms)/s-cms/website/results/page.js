...
import { 
  ResultsHeroEditor, 
  TopRankersEditor, 
  SubjectPerformanceEditor, 
  SuccessStoriesEditor, 
  AwardsEditor 
} from "@/cms/components/cms/resultspage";

const DEFAULT_DATA = {
  hero: { badge: "", title: "", description: "" },
  years: ["2026", "2025", "2024"],
  categories: ["SSC", "HSC Science", "HSC Commerce", "Foundation"],
  topRankers: [],
  subjectPerformance: [],
  successStories: [],
  awards: []
...
export default function ResultsCMSPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Failed to save results data:", err);
    }
  ...
  return (
    <PageContainer>
      <div className="flex flex-col lg:flex-row gap-6 mb-8 items-start lg:items-center justify-between animate-fade-in-up">
        <div>
          <h1 className="text-2xl font-black text-navy uppercase tracking-tight">Manage Results</h1>
          <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">
            Edit the Results page content and statistics
          </p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="flex-1 max-w-4xl min-w-0">
          <div className="space-y-6">
            ...
            {activeTab === "rankers" && (
              <TopRankersEditor 
                data={data.topRankers} 
                categories={data.categories}
                years={data.years}
                onChange={(newData) => handleDataChange("topRankers", newData)} 
              />
            )}
            ...
            {activeTab === "subjects" && (
              <SubjectPerformanceEditor 
                ...
                onChange={(newData) => handleDataChange("subjectPerformance", newData)} 
              />
            )}
            ...
            {activeTab === "stories" && (
              <SuccessStoriesEditor 
                ...
                onChange={(newData) => handleDataChange("successStories", newData)} 
              />
            )}
            ...
            {activeTab === "awards" && (
              <AwardsEditor 
                data={data.awards} 
                onChange={(newData) => handleDataChange("awards", newData)} 
              />
            )}
